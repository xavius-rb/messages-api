...
RSpec.describe Messages::Api::Configuration do
  ...
  describe "#validate!" do
    context "when required fields are missing" do
      ...
      it "raises ArgumentError when api_secret is missing" do
        config.api_key = "key"
        config.base_uri = "uri"
        expect { config.validate! }.to raise_error(ArgumentError, "API secret is required")
      end

      it "raises ArgumentError when base_uri is missing" do
        config.api_key = "key"
        config.api_secret = "secret"
        expect { config.validate! }.to raise_error(ArgumentError, "Base URI is required")
      end
    end
